...
1. 点击“设置”。
2. 点击展开“模型服务配置”。
3. 如使用官方 OpenAI 地址，可留空 Base URL 和 API Key，系统会优先使用 Vercel 环境变量。
4. 如填写自定义 Base URL，需要同时填写该服务对应的 API Key。
5. 按需填写模型名。
6. 点击“保存设置”。
7. 点击“联通测试”。
8. 预期：配置正确时显示联通成功；配置缺失或错误时显示失败原因。
9. 刷新页面后再次进入设置页。
10. 预期：配置仍保存在当前浏览器。
11. 点击展开“关于平台”。
12. 预期：显示平台版本、文件范围、部署形态和数据说明。
...
- DOCX 上传是否成功。
- `/api/analyze` 是否能调用模型。
- 官方 OpenAI 地址未填写页面 Key 时，是否能使用 Vercel 环境变量调用模型。
- 自定义 Base URL 未填写 Key 时，是否会提示补充 API Key。
- 自定义 OpenAI-compatible Base URL 是否能通过 Chat Completions 路径完成联通测试和合同分析。
- Excel 是否能下载。
- 刷新页面后本地历史记录是否仍在。
...
当前 Demo 已将合同文本请求上限提高到 300000 字符，并将自定义 OpenAI-compatible 服务的输出上限调整为 `max_tokens=8192`。若真实合同继续出现只分析开头的问题，建议升级为分段抽取流程。
